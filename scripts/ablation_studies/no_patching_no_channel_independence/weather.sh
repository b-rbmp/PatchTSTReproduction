...
if [ ! -d "./logs/ablation_original" ]; then
    mkdir ./logs/ablation_original
fi
...
for prediction_length in 336 720
do
    python -u src/training/supervised/train.py \
      ...
      --patience 20\
      --only_patching \
      --bootstrap_iterations 1 --batch_size 1 --learning_rate 0.0001 >logs/ablation_original/$model_identifier'_'$input_length'_'$prediction_length.log 
done